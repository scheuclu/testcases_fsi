...
if 'NUMANGLES' in locals() and 'NUMPERTURB' in locals() and 'NUMMACH' in locals():
...
for index_mach in range(1,NUMMACH+1):
    for index_angle in range(1,NUMANGLES+1):
        resultfile = open('results/fdsim_'+str(index_mach)+'_'+str(index_angle)+'.csv','w')
        resultfile.write("absvar-value,dLx,dLy\n")
        for index_perturb in range(1,NUMPERTURB+1):
            simname = "fdsim_"+str(index_mach)+'_'+str(index_angle)+"_"+str(index_perturb)
            print(simname)
            ...
            with open(simname+'/results/LiftAndDrag_steady_minus.out') as f:
                s = f.readlines()
                lastrow = s[-1]
                split   = lastrow.split()
